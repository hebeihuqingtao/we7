...
(new yii\web\Application($config))->run();' ;
    ...
    //修改PDO.php首页
    $index_file = ROOT_PATH.'./web/pdo.php';
    $a='pdo';
    $index_content =<<<EOT
<?php
$$a=new PDO('mysql:host=$location;port=$pore;dbname=$db_name','$db_user','$db_pwd',array(PDO::MYSQL_ATTR_INIT_COMMAND=>'set names utf8'));
EOT;
    $is_success = file_put_contents($index_file,$index_content);
    if(!$is_success)
    {
        showProgress(array('isError' => true,'message' => '生成pdo.php页面出错','percent' => 0.9));
    }
